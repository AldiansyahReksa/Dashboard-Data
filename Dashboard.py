...
if df.empty:
    st.error("Data kosong atau tidak valid.")
    st.stop()
...
# Validasi jika tidak ada data untuk pilihan pintu masuk
if pintu_pilihan not in df["Pintu Masuk"].values:
    st.error(f"Tidak ada data untuk pintu masuk: {pintu_pilihan}")
    st.stop()
