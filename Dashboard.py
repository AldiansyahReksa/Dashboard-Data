...
# Filter nama jalur spesifik (menghapus total dan pintu lainnya)
def is_specific_jalur(pintu):
    return not ("Total" in pintu or "Pintu Lainnya" in pintu)

df_specific = df[df["Pintu Masuk"].apply(is_specific_jalur)]
...
jalur_tersedia = df_specific[df_specific["Jalur"] == jalur_pilihan]["Pintu Masuk"].unique()
